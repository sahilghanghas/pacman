...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # termination condition when ghost is in jail
        ...
        '''	
        error: counter object not callable
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        # how to iterate over old pos newpos and prob
        # new postion would depend on probability of finding at t+1 position and beliefs of old position
        for p in self.legalPositions:
			newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,p))
			for newPos, prob in newPosDist.items():
				# prob (t+1 location | old position
				allPossible[newPos] += prob * self.beliefs[p]
				# error: only returning a single value -> summation with previous values
				# still returning same values
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        '''
        allPossible = util.Counter()
        # 1st special case **** jail condition ****
        if noisyDistance == None:
	        ...
	        if allPossible.totalCount() == 0:
	        	...
	        	for _ in range(0,self.numParticles):
	        		part.append(util.sample(allPossible))
	        	self.particles = part
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
        	newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        	distribution.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        permutationsOfLegalPositions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(permutationsOfLegalPositions)
        # use counter to append permutation of legal ghost positions in self particles
        counter = 0
        #declare self.particles
        self.particles = []
        while counter < self.numParticles:
        	# iterate over all random positions
        	for permutations in permutationsOfLegalPositions:
        		if counter < self.numParticles:
        			#add positions into particles
        			self.particles.append(permutations)
        			counter += 1
        		else:
    ...
    def observeState(self, gameState):
        ...
        allPossible = util.Counter()
        #iterate over index
        #since self.particles have a counter associated with it we can use enumerate built in python function
